...
import "server-only";
import {
    getHabitReminderHtml,
    getHabitReminderText,
    type HabitReminderTemplateData,
} from "./email/templates/habit-reminder";
import {
...
export async function sendHabitReminderEmail(
    email: string,
    templateData: HabitReminderTemplateData,
) {
    return sendEmail({
        to: email,
        subject: "Your Habitly check-in is waiting",
        html: getHabitReminderHtml(templateData),
        text: getHabitReminderText(templateData),
    });
}
